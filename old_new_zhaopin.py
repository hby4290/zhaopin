...
# 以下是数据文件路径 csv格式
data = pd.read_csv('D:\zhaopin\PLUS_M1.csv', encoding='gbk')
print(data.shape, data.columns)
# 以下是figure（整体画面）的设置
fig = plt.figure(figsize=(8, 6), facecolor='whitesmoke')
# plt.figure 图形，在这里可以理解为画板，在画板上绘制图标。参数为设定大小 facecolor为设置背景颜色，这里用的是白烟色，相比纯白色增加了一些灰度
# 新老用户比例
# 标签内容
labels = data['is_1st'].value_counts().index
# 偏移扇区
explodes = [0.05 if i == 1 else 0 for i in labels]
# 设置颜色 颜色最好还是自己写 饼图会根据所给出的颜色进行循环使用。
colors = "m", "blue"
patchs, l_text, p_text = plt.pie(data['is_1st'].value_counts(), labels=["曾经发布过招聘信息", "第一次发布招聘信息"], autopct="%1.1f%%", colors=colors,
...
for i, p in enumerate(patchs):
    p.set_alpha(0.4)
# 设定字
plt.title('第一次发布招聘信息占比', fontproperties=font)
